...
void loadBrightnessFromPreferences(int value) {
    ...
    lv_label_set_text_fmt(objects.brightness_value_label, "%d%%", value);
...
void loadVolumeFromPreferences(int value) {
    ...
    //update ui
    lv_arc_set_value(objects.volume_arc, value);
    lv_label_set_text_fmt(objects.volume_value_label, "%d%%", value);
    Serial.println("Loaded volume: " + String(value));
}
void loadRotationFromPreferences(bool value) {
    shouldRotate = value;
    if (shouldRotate) {
        lv_obj_add_state(objects.rotate_button, LV_STATE_CHECKED);
    } else {
        lv_obj_clear_state(objects.rotate_button, LV_STATE_CHECKED);
    }
